...
export default function CreateImage() {
  ...
  const [selectedStyle, setSelectedStyle] = useState('照片级写实场景'); // 新增风格状态，默认使用nano_styles的第一个
  ...
  const styles = [
    ...
    // 定义风格选项
  const nano_styles = ['无',
    '照片级写实场景', '油画风格', '日系动漫插画风格', '卡哇伊风格',"吉卜力风格（宫崎骏风）","黑暗动漫","漫画风格", '极简风格', 
    '复古科学插画风格', '艺术黑白肖像', '梵高风格油画', '胶片颗粒质感', '干净线条与平涂色彩',
    '等距3D模型插画', '科学信息图风格', '现代极简', '工作室灯光商业摄影', '黑暗粗糙艺术'
  ];

  const styles_eng: Record<string, string> = {
    '无': '无',
    '照片级写实场景': 'photorealistic',
    '油画风格': 'oil painting style',
    '日系动漫插画风格': 'Japanese anime style',
    '卡哇伊风格': 'Kawaii anime style',
    '吉卜力风格（宫崎骏风）': 'Studio Ghibli style',
    '黑暗动漫': 'Dark anime style',
    '漫画风格': 'Manga illustration style',
    '极简风格': 'minimalist style',
    '复古科学插画风格': 'vintage scientific illustration style',
    '艺术黑白肖像': 'artistic black and white portrait',
    '梵高风格油画': 'Vincent van Gogh style oil painting',
    '胶片颗粒质感': 'film grain texture',
    '干净线条与平涂色彩': 'clean lines, flat colors',
    '等距3D模型插画': 'isometric 3D model illustration',
    '科学信息图风格': 'scientific information graph style',
    '现代极简': 'modern minimalist style',
    '工作室灯光商业摄影': 'studio lighting commercial photography',
    '黑暗粗糙艺术': 'gritty, noir art style'
  };


  
  
/*经典日系动漫Japanese anime style"生成一张经典日系动漫风格图像：一个高中少女在樱花树下奔跑，穿着校服，大眼睛闪亮，动态姿势，背景有飘落的粉色花瓣，线条流畅，色彩鲜艳，高分辨率。"
"Create a Japanese anime style scene of a mecha robot battling in a futuristic city, exaggerated expressions, vibrant neon colors, detailed mechanical designs and explosions."
卡哇伊动漫（可爱风）Kawaii anime style"卡哇伊动漫风格的插画：一只萌萌的小猫穿着围裙烘焙蛋糕，圆润身体，大头大眼睛，背景是糖果和星星，柔和粉色调，简单可爱线条。"
"Kawaii anime style portrait of a chibi character with oversized head, sparkling eyes, pastel colors, cute accessories like bows and hearts.
"吉卜力风格（宫崎骏风）Studio Ghibli style"以吉卜力风格生成图像：一个女孩骑着扫帚飞过森林，背景是茂密绿树和魔法生物，手绘水彩质感，温暖自然色调，梦幻氛围，像《千与千寻》。"
"Studio Ghibli style landscape of a floating castle in the clouds, whimsical creatures, soft lighting, detailed foliage and ethereal elements.
"漫画风格（Manga）Manga illustration style"漫画风格的黑白图像：一个忍者跃起攻击姿势，速度线和动态效果，精细墨线，夸张肌肉和表情，背景简洁，像《火影忍者》风格。"
"Manga style comic panel of a romantic scene between two characters, speech bubbles optional, bold outlines, shading with screentones.
"现实主义动漫Realistic anime style"现实主义动漫风格的肖像：一个年轻侦探在雨中站立，细致面部特征和头发纹理，混合照片级细节与动漫眼睛，阴郁蓝灰色调，高细节。"
"Generate a realistic anime style full-body image of a cyberpunk hacker, lifelike proportions, subtle anime influences in eyes and hair, neon-lit urban background.
"黑暗动漫（Noir/暗黑风）Dark anime style"黑暗动漫风格的场景：一个孤单身影走在废弃城市街道，阴影重叠，夸张表情和血红眼睛，哥特元素，深色调和高对比度，像《死亡笔记》。"
"Dark anime style illustration of a vampire in a moonlit forest, intense gaze, dramatic shadows, crimson accents and intricate clothing details."
这些风格可以混合使用，如“Japanese anime with Studio Ghibli elements”。如果结果不理想，添加“high detail, vibrant colors”或参考具体作品来优化。 建议在提示中避免过于复杂场景，以获得更好一致性。
*/
    ...
    if (savedHistory) {
      try {
        ...
        const historyWithDates = parsedHistory.map((item: any) => ({
          ...
          // 如果历史记录中没有style字段，则设置默认值
          style: item.style || '现实主义',
  ...
  const callImageGenerationApi = async (): Promise<void> => {
    ...
    let request_prompt = prompt.trim();
    if (selectedStyle !== '无' && selectedStyle !== "") {
        if (styles_eng[selectedStyle]) {
          request_prompt = `${styles_eng[selectedStyle]}: ${prompt}`;
        } else {
          request_prompt = `${selectedStyle}: ${prompt}`;
        }
    }
    ...
    // 根据选择的模型使用不同的API端点
    const apiEndpoint = selectedModel === NANO_BANANA_MODEL ? NANO_BANANA_API_ENDPOINT : GPT_IMAGE_1_API_ENDPOINT;
  ...
  const handleSelectFromHistory = (imageId: string) => {
    ...
    if (selectedImage) {
      ...
      setSelectedStyle(selectedImage.style); // 恢复风格设置
  ...
  // 添加一个新的useEffect来处理模型切换时的风格选择
  useEffect(() => {
    // 当模型切换为nano-banana时，默认选择第一个nano_style
    if (selectedModel === NANO_BANANA_MODEL && nano_styles.includes(selectedStyle)) {
      // 如果当前选中的风格在nano_styles中，保持不变
      return;
    } else if (selectedModel === NANO_BANANA_MODEL) {
      // 如果切换到nano-banana模型，且当前风格不在nano_styles中，则设置为第一个nano_style
      setSelectedStyle(nano_styles[0]);
    } else if (selectedModel === GPT_IMAGE_1 && styles.includes(selectedStyle)) {
      // 如果当前选中的风格在styles中，保持不变
      return;
    } else if (selectedModel === GPT_IMAGE_1) {
      // 如果切换到gpt-image-1模型，且当前风格不在styles中，则设置为第一个style
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          >
                            {/* 根据选择的模型显示对应的风格选项 */}
                            {(selectedModel === NANO_BANANA_MODEL ? nano_styles : styles).map((style) => (
                  ...
                  {selectedHistory && (
                    <div className={`rounded-lg p-4 ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                      ...
                      <div className="text-sm space-y-1">
                        ...
                        <p><span className="font-medium">风格:</span> {selectedHistory.style}</p>
