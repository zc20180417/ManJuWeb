...
export default function ScriptWriter() {
  ...
  const [aiModels] = useState([
    { id: 'doubao1-6-thinking', name: '豆包1.6（深度思考响应慢）', apiEndpoint: 'https://ark.cn-beijing.volces.com/api/v3/chat/completions' },
    { id: 'doubao1-6-flash', name: '豆包1.6 (flash响应快)', apiEndpoint: 'https://ark.cn-beijing.volces.com/api/v3/chat/completions' },
    { id: 'gpt4', name: 'GPT-4', apiEndpoint: 'https://api.openai.com/v1/chat/completions' },
  ...
  const [selectedModel, setSelectedModel] = useState('doubao1-6-flash');
  ...
  const realApiCall = async (model: string, novelText: string, projectTitle: string): Promise<void> => {
    ...
    if (model === 'doubao1-6-thinking' || model === "doubao1-6-flash") {
      // 火山方舟豆包模型的请求格式
      let real_model = model == 'doubao1-6-thinking' ? "doubao-seed-1-6-thinking-250715" : "doubao-seed-1-6-flash-250828";
      requestBody = {
        model: real_model,
        messages: [
          {
            ...
            content: [
              {
                type: "text",
                text: `请将以下小说文本转换为专业剧本格式。小说标题：${projectTitle}，内容：${novelText}。请确保剧本包含场景、角色对话、动作描述等元素。`
              }
            ]
          }
        ],
    ...
    try {
      ...
      // 根据不同模型解析响应
      if (model === 'doubao1-6-flash' || model === "doubao1-6-thinking") {
        // 豆包模型响应解析
      ...
      } else if (model === 'gpt4' || model === 'claude3' || model === 'gemini' || model === 'tongyi') {
        // 其他模型响应解析
        if (data.choices && data.choices[0] && data.choices[0].message) {
          generatedScript = data.choices[0].message.content;
